...
- **Existing tests may reveal missing implementation modules.** If an existing or planned test imports a local module that is absent from the target tree, include the missing implementation file in `impacted_files` so codegen is allowed to create it.
